...
Modularized faucet for EVM chains with different protection methods (Captcha, Mining, IP, Mainnet Balance, Gitcoin Passport, **SIWE Authentication** and more)
...
This faucet is mostly known for its proof-of-work based protection, which is currently the best and most reliable way to distribute funds on a network that got low on fund reserves. **Additionally, it now supports SIWE (Sign-In with Ethereum) authentication**, allowing users to verify wallet ownership and receive enhanced rewards.
...
## 🔐 SIWE Authentication (NEW!)
- **Sign-In with Ethereum** support following EIP-4361 standard
- **Enhanced rewards** for authenticated users (configurable bonus multiplier)
- **Auto-address completion** after wallet authentication
- **Wallet-based restrictions** and limits
- **Modern UI** with MetaMask integration
- **Secure nonce-based** authentication without on-chain transactions

## 🛡️ Multiple Protection Methods
- **Proof-of-Work Mining** - Most effective anti-bot protection
- **Captcha Verification** - Human verification challenges
- **IP-based Limits** - Geographic and network restrictions
- **Mainnet Balance Checks** - Verify real ETH holdings
- **Gitcoin Passport** - Decentralized identity verification
- **ENS Name Resolution** - Support for Ethereum Name Service
- **Time-based Restrictions** - Cooldown periods and rate limiting
...
- **Configurable Rewards** - Flexible payout amounts and conditions
- **Session Management** - Persistent user sessions with progress tracking
...
To enable SIWE authentication in your faucet, add this to your `faucet-config.yaml`:

```yaml
modules:
  siwe:
    enabled: true
    domain: "your-faucet-domain.com"  # Must match your public domain
    uri: "https://your-faucet-domain.com"
    nonceExpiration: 300  # 5 minutes
    sessionExpiration: 86400  # 24 hours
    required: false  # Make optional initially
    rewardFactor: 1.5  # 50% bonus for authenticated users
    restrictions:
      - limitCount: 5
        duration: 86400  # Daily limit per authenticated wallet
```

For detailed SIWE configuration, see: [SIWE Setup Guide](./docs/guides/siwe-setup.md)

## Documentation

- 📖 [SIWE Module Documentation](./docs/modules/siwe.md) - Complete SIWE feature guide
- 🚀 [SIWE Setup Guide](./docs/guides/siwe-setup.md) - Step-by-step configuration
- 💡 [Ideas and Improvements Plan](./docs/PLAN_IDEAS_MEJORAS.md) - Roadmap and future features
- 🔧 [Module Development](./docs/guides/) - Additional configuration guides
...
**SIWE Module Contributors:**
- **Luis Corales** - SIWE authentication implementation, modern UI improvements, Spanish localization, and comprehensive testing suite
